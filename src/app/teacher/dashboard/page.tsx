...
export default function TeacherDashboardPage() {
  ...
  return (
    <DashboardShell>
      <header className="flex flex-col gap-5 border-b border-white/10 pb-6 sm:flex-row sm:items-end sm:justify-between sm:gap-6 sm:pb-8">
        <div>
          <p className="text-[10px] uppercase tracking-[0.42em] text-[#78b8ff]">Teacher dashboard</p>
          <h1 className="mt-3 text-3xl font-bold tracking-[-0.06em] sm:text-6xl">Welcome</h1>
        </div>
        <button onClick={logOut} className="min-h-11 self-start rounded-full border border-white/15 px-5 py-3 text-[10px] font-bold uppercase tracking-[0.18em] text-white/65 transition hover:border-[#2387ff]/50 hover:text-white">Log out</button>
      </header>
      ...
      <section className="mt-8 rounded-[2rem] border border-white/10 bg-[#0a1b2e]/80 p-4 shadow-[0_0_80px_rgba(35,135,255,0.06)] sm:mt-10 sm:p-7">
        ...
        <div className="mt-6 grid gap-3 sm:mt-7 lg:flex">
          ...
          <select value={track} onChange={(event) => { setTrack(event.target.value); setPage(0); }} className="w-full rounded-xl border border-white/10 bg-[#071321]/80 px-4 py-3 text-sm text-white outline-none focus:border-[#2387ff]/60 lg:w-auto">{trackOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          <select value={experience} onChange={(event) => { setExperience(event.target.value); setPage(0); }} className="w-full rounded-xl border border-white/10 bg-[#071321]/80 px-4 py-3 text-sm text-white outline-none focus:border-[#2387ff]/60 lg:w-auto">{experienceOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
        </div>
      ...
      {toast && <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 right-4 z-40 flex items-center justify-between gap-3 rounded-2xl border border-[#2387ff]/35 bg-[#0a1b2e]/95 px-4 py-3 text-sm text-white shadow-[0_15px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:rounded-full sm:px-5"><span>Registration marked as read.</span><button onClick={() => void undoToast()} className="min-h-10 shrink-0 font-bold uppercase tracking-[0.16em] text-[#78b8ff] transition hover:text-white">Undo</button></div>}
    </DashboardShell>
  );
}
...
function RegistrationModal({ registration, tab, onClose, onStatusChange, busy }: { registration: Registration; tab: Tab; onClose: () => void; onStatusChange: () => void; busy: boolean }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#020810]/80 p-3 backdrop-blur-sm sm:p-5" role="presentation" onClick={onClose}><div role="dialog" aria-modal="true" aria-label="Registration details" onClick={(event) => event.stopPropagation()} className="max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-[1.5rem] border border-[#2387ff]/30 bg-[#0a1b2e] p-5 shadow-[0_30px_120px_rgba(0,0,0,0.5)] sm:max-h-[90vh] sm:rounded-[2rem] sm:p-9"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.4em] text-[#78b8ff]">Registration</p><h2 className="mt-3 text-2xl font-bold tracking-[-0.05em] sm:text-3xl">{registration.student_name}</h2><p className="mt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-[#8bc3ff]">{registration.is_read ? "Read / Archived" : "Unread"}</p></div><button onClick={onClose} className="min-h-10 rounded-full border border-white/15 px-4 py-2 text-[10px] uppercase tracking-[0.18em] text-white/60">Close</button></div><div className="mt-7 grid gap-4 sm:mt-8 sm:grid-cols-2 sm:gap-5">{[["Parent / Guardian", registration.parent_name], ["Age", String(registration.student_age)], ["Preferred Track", trackLabel(registration.preferred_track)], ["Email", registration.email], ["Phone / WhatsApp", registration.phone], ["Prior MUN Experience", experienceLabel(registration.prior_mun_experience)], ["How they heard about us", referralLabel(registration.heard_from)], ["Original registration date", formatDate(registration.created_at)]].map(([label, value]) => <div key={label}><p className="text-[9px] uppercase tracking-[0.2em] text-white/35">{label}</p><p className="mt-2 break-words text-sm text-white/80">{value}</p></div>)}</div><div className="mt-6 border-t border-white/10 pt-5 sm:mt-7 sm:pt-6"><p className="text-[9px] uppercase tracking-[0.2em] text-white/35">Additional information</p><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7 text-white/65">{registration.additional_info || "Not provided"}</p></div><div className="mt-7 flex flex-col-reverse gap-3 border-t border-white/10 pt-5 sm:mt-8 sm:flex-row sm:justify-end sm:pt-6"><button onClick={onClose} className="min-h-11 rounded-full border border-white/15 px-5 py-3 text-[10px] uppercase tracking-[0.18em] text-white/60">Close</button>{tab === "inbox" ? <button disabled={busy} onClick={onStatusChange} className="min-h-11 rounded-full bg-[#2387ff]/20 px-5 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[#8bc3ff] disabled:opacity-50">{busy ? "Saving..." : "Mark as read ✓"}</button> : <button disabled={busy} onClick={onStatusChange} className="min-h-11 rounded-full border border-[#2387ff]/35 px-5 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[#8bc3ff] disabled:opacity-50">{busy ? "Saving..." : "Restore to inbox"}</button>}</div></div></div>;
}

function DeleteModal({ onCancel, onConfirm, busy }: { onCancel: () => void; onConfirm: () => void; busy: boolean }) {
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-[#020810]/85 p-3 backdrop-blur-sm sm:p-5" role="presentation"><div role="alertdialog" aria-modal="true" aria-labelledby="delete-title" className="w-full max-w-md rounded-[1.5rem] border border-red-300/20 bg-[#0a1b2e] p-5 shadow-[0_30px_120px_rgba(0,0,0,0.55)] sm:rounded-[2rem] sm:p-7"><p className="text-[10px] uppercase tracking-[0.35em] text-red-200/70">Permanent action</p><h2 id="delete-title" className="mt-4 text-2xl font-bold tracking-[-0.05em] sm:text-3xl">Delete this registration permanently?</h2><p className="mt-4 text-sm leading-6 text-white/50">This cannot be undone.</p><div className="mt-7 flex flex-col-reverse gap-3 sm:mt-8 sm:flex-row sm:justify-end"><button onClick={onCancel} className="min-h-11 rounded-full border border-white/15 px-5 py-3 text-[10px] uppercase tracking-[0.18em] text-white/65">Cancel</button><button disabled={busy} onClick={onConfirm} className="min-h-11 rounded-full bg-red-200 px-5 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#321018] disabled:opacity-50">{busy ? "Deleting..." : "Delete permanently"}</button></div></div></div>;
}
